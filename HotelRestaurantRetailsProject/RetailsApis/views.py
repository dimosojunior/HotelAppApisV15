...
class AllRetailsCategoriesView(APIView):
# Eg: http://127.0.0.1:8000/Retails/AllRetailsCategories/?id=2
    def get(self, request):
        try:
            
            categoryId = int(request.query_params.get('id'))
            
            queryset = RetailsCategories.objects.filter(
                Inventory__id__icontains = categoryId
                )

            serializer = RetailsCategoriesSerializer(queryset, many=True)
...
class RetailsProductsViewSet(APIView):
    def get(self, request):
        try:
            ...
            serializer = RetailsProductsSerializer(page_items, many=True)
...
class RetailsInventoryViewSet(APIView):
    def get(self, request):
        try:
            ...
            return Response({'error': str(e), "queryset":[]}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
class RetailsCategoriesViewSet(APIView):
    def get(self, request):
        try:
            # Get the page number from the query parameters, default to 1
            page = int(request.query_params.get('page', 1))
            ...
            serializer = RetailsCategoriesSerializer(page_items, many=True)
